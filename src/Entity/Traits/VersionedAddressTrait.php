...
trait VersionedAddressTrait
{
    ...
    /**
     * Gets the address components as an array.
     *
     * @param bool $includePobox
     * @return array
     */
    public function getAddress($includePobox = true)
    {
        return $includePobox ?
            [$this->streetAddress, str_pad($this->zipcode, 5, '0', STR_PAD_LEFT), $this->city, $this->pobox, $this->country] :
            [$this->streetAddress, str_pad($this->zipcode, 5, '0', STR_PAD_LEFT), $this->city, $this->country];
    }

    public function getAddressAsString()
    {
        $address = $this->streetAddress . ', ' . str_pad($this->zipcode, 5, '0', STR_PAD_LEFT) . ' ' . $this->city;
        if (!empty($this->pobox))
            $address .= ', ' . $this->pobox;
