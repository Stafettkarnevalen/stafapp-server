...
use App\Controller\Apple\ApplePushNotificationsController;
...
class AuthenticatedUserController extends FOSRestController implements ModalEventController
{
    ...
    private $profileTypes = [
        'notifications' => ['bell', 'profile.notifications', 'profile.info_notifications', 'nav.authuser_view_profile', 'security/user/notifications.html.twig'],
        'tooltips' => ['question-circle', 'profile.tooltips', 'profile.info_tooltips', 'nav.authuser_view_profile', 'security/user/tooltips.html.twig'],
        'modules' => ['th-large', 'profile.modules', 'profile.info_modules', 'nav.authuser_view_profile', 'security/user/modules.html.twig'],
    ];
    ...
    private $notificationTypes = [
        'system' => ['exclamation-circle', 'profile.notification_types.system', 'profile.notification_types.system_info', true],
        'reminders' => ['bell-o', 'profile.notification_types.reminders', 'profile.notification_types.reminders_info', false],
        'news' => ['newspaper-o', 'profile.notification_types.news', 'profile.notification_types.news_info', false],
        'messages' => ['commenting-o', 'profile.notification_types.messages', 'profile.notification_types.messages_info', false],
    ];

    /** @var array Paths for different browsers or platforms */
    private $notificationPaths = [
        'Safari' => 'ApplePushNotifications.Safari.notificationTypes',
        'Chrome' => 'GooglePushNotifications.Chrome.notificationTypes',
    ];
     ...
     * @Route("/{_locale}/authenticated/users/profile/view/{type}",
     *     options={"expose"=true},
     ...
     * @return JsonResponse|Response
     */
    public function authUserListProfileAction($type = 'notifications', Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        ...
        $fb = $this->createFormBuilder($usr->getData($this->notificationPaths[$browser->getName()] . '.notificationTypes', []), [
            'translation_domain' => 'user',
            'attr' => ['action' => $request->getPathInfo()]
        ]);
        foreach ($this->notificationTypes as $ntype => $data) {
            $fb->add($ntype, ChoiceType::class, [
                'label' => $data[1],
                'expanded' => true,
                ...
                'disabled' => $data[3],
                'data' => $data[3] ? 1 : $usr->getData($this->notificationPaths[$browser->getName()] . '.notificationTypes.' . $ntype, 1)
            ]);
        }
        ...
        $fb->add('test', SubmitType::class, [
            'left_icon' => 'fa-comment-o',
            'right_icon' => 'fa-send',
            'attr' => ['class' => 'btn-primary'],
            'label' => 'action.test_push',
            'translation_domain' => 'user',
        ]);
        $form = $fb->getForm();
        $form->handleRequest($request);
        ...
        /** @var SubmitButton $test */
        $test = $form->get('test');

        if ($form->isSubmitted() && $save->isClicked()) {
            $data = $form->getData();
            $data['system'] = 1;

            $profile->setDataForPath($this->notificationPaths[$browser->getName()] . '.' . 'notificationTypes', $data);
            $em->merge($profile);
            $em->flush();
        } else if ($form->isSubmitted() && $test->isClicked()) {
            /** @var Kernel $kernel */
            $kernel = $this->get('kernel');

            $cert = new Certificate(file_get_contents($kernel->getProjectDir() . ApplePushNotificationsController::PEM_PATH));
            $client = new Client(new SocketClient($cert, ApplePushNotificationsController::APNS_SERVER, 2195));
            ...
            $sender->send($token, 'Testing StafApp', 'This is a test message',  $request->getLocale() . '/authenticated/users/profile/view');
